...
interface ScheduleEditErrorProps {
  error: Error & { digest?: string }
  reset: () => void
}

export default function ScheduleEditError({ error, reset }: ScheduleEditErrorProps) {
  const params = useParams<{ mouId: string }>()
  const mouId = params?.mouId ?? ''

  useEffect(() => {
    // eslint-disable-next-line no-console
    console.error('[mous-schedule-edit-error]', error)
  }, [error])
  ...
  return (
    <main id="main-content" className="mx-auto max-w-screen-sm px-4 py-24">
      <h1 className="font-heading text-2xl font-semibold text-brand-navy">
        Something went wrong loading the schedule editor.
      </h1>
      ...
      <div className="mt-5 flex gap-3">
        ...
        <Link
          href={mouId ? `/mous/${mouId}/installments` : '/mous'}
