...
interface MousNewErrorProps {
  error: Error & { digest?: string }
  reset: () => void
}

export default function MousNewError({ error, reset }: MousNewErrorProps) {
  useEffect(() => {
    // eslint-disable-next-line no-console
    console.error('[mous-new-error]', error)
  }, [error])
  ...
  return (
    <main id="main-content" className="mx-auto max-w-screen-sm px-4 py-24">
      ...
      <p className="mt-3 text-sm text-foreground">
        Refresh to try again, or pick a different template. If the problem
        repeats, send the diagnostic id below to Anish.
      </p>
      {error.digest ? (
