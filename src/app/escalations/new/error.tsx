...
interface EscalationNewErrorProps {
  error: Error & { digest?: string }
  reset: () => void
}

export default function EscalationNewError({ error, reset }: EscalationNewErrorProps) {
  useEffect(() => {
    // eslint-disable-next-line no-console
    console.error('[escalation-new-error]', error)
  }, [error])
  ...
  return (
    <main id="main-content" className="mx-auto max-w-screen-sm px-4 py-24">
      <h1 className="font-heading text-2xl font-semibold text-brand-navy">
        Something went wrong loading the escalation form.
      </h1>
