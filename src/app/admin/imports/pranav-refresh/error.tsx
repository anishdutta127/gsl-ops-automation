...
interface PranavRefreshErrorProps {
  error: Error & { digest?: string }
  reset: () => void
}

export default function PranavRefreshError({ error, reset }: PranavRefreshErrorProps) {
  useEffect(() => {
    // eslint-disable-next-line no-console
    console.error('[pranav-refresh-error]', error)
  }, [error])
  ...
  return (
    <main id="main-content" className="mx-auto max-w-screen-sm px-4 py-24">
      <h1 className="font-heading text-2xl font-semibold text-brand-navy">
        Something went wrong loading the Pranav refresh tool.
      </h1>
