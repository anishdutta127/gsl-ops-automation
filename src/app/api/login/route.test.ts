...
describe('POST /api/login', () => {
  it('happy path: 303 redirect to the role /work board (Step 4 landing) with Set-Cookie', async () => {
    mockAuthn.mockResolvedValue({ ok: true, user: fakeUser() })
    const res = await POST(
      buildRequest({ email: 'anish.d@example.test', password: 'GSL#123' }),
    )
    expect(res.status).toBe(303)
    expect(res.headers.get('location')).toBe('http://localhost/work/admin')
  ...
  it('happy path with INVALID next falls back to the role /work board, cookie still set', async () => {
    mockAuthn.mockResolvedValue({ ok: true, user: fakeUser() })
    const res = await POST(
      buildRequest({
        email: 'anish.d@example.test',
