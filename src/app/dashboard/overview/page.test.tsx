/*
 * /dashboard/overview tests (originally /page.test.tsx, Gate 3.6).
 *
 * Phase 6F (2026-05-21) moved the legacy 5-zone landing from `/` to
 * `/dashboard/overview` to make room for the action-first homepage.
 * These tests follow the relocation and assert the same surface at
 * the new URL.
 */
...
describe('/dashboard/overview consolidated landing', () => {
  it('redirects unauthenticated callers to /login', async () => {
    getCurrentUserMock.mockResolvedValue(null)
    const { default: HomePage } = await import('./page')
    await expect(HomePage()).rejects.toThrow('REDIRECT:/login?next=%2Fdashboard%2Foverview')
  })
