...
**Until Gate 5 cutover:** mou-system continues to issue PIs and increment its `pi_counter.json` (the per-GSTIN shape `{fiscalYear, entities: { MH: {next}, UP: {next} }}`). Pranav, Shubhangi, and Anita continue daily PI work on `gsl-mou-system.vercel.app`.

**Ops platform during parallel-build:** Gate 2 Step 3 captured the mou-system counter snapshot at `src/data/pi_counter_map.json` (live values: fiscalYear `2627`, MH `next: 2`, UP `next: 16` as of the snapshot date). The mouSystem-namespace lib `src/lib/mouSystem/piCounterAtomic.ts` reads + writes this file, NOT Ops's existing `src/data/pi_counter.json`. The two counter files co-exist:

- `src/data/pi_counter.json` (Ops legacy, single-counter `{fiscalYear, next, prefix}`): consumed by `src/lib/githubQueue.ts issuePiNumberAtomic`, `src/lib/pi/generatePi.ts`, `src/lib/dispatch/raiseDispatch.ts`, `src/app/api/dispatch/[id]/dispatch-note/route.ts`, `src/lib/deliveryAck/generateDeliveryAck.ts`. These libs continue serving the parallel-build verification flows.
- `src/data/pi_counter_map.json` (mou-system per-entity, snapshot from cutover-source-of-truth): consumed by `src/lib/mouSystem/piCounterAtomic.ts`. Ready to take over at cutover.

The two files do NOT contend during parallel-build because each is read by a distinct lib. Ops doesn't issue production PIs through the mouSystem stack during this window; any PI generated through that stack is a verification artefact (per §5).

`piCounterAtomic.ts` handles the GitHub Contents API SHA-conflict retry pattern: read-modify-write with up to 3 retries on 409. The `migrateLegacyCounter` helper accommodates both the legacy single-counter shape and the new per-entity map, parking any stray legacy `next` value on the UP entity (which historically held the only counter).

**At cutover:** the counter state is read one final time from mou-system and locked into `src/data/pi_counter_map.json`. From cutover instant onward, Ops is the only system that issues PIs; Ops's existing dispatch / delivery-ack / pi-generate libs migrate to consume the per-entity shape (or the mouSystem libs become the single PI surface), and `src/data/pi_counter.json` retires.
...
## 10. Company config consolidation (Gate 2 §3)

**Decision: keep co-located until cutover.** Ops's `config/company.json` is the GSL-Ops single-entity shape (`Get Set Learn Private Limited`, GSTIN `29AAAAA0000A1Z5`, sample bank details, paymentTerms, gstRate). It is consumed by Ops's existing dispatch flow (`src/lib/dispatch/raiseDispatch.ts`, `src/app/api/dispatch/[id]/dispatch-note/route.ts`), Ops's existing PI lib (`src/lib/pi/generatePi.ts`, `src/lib/pi/templates.ts`), and the delivery-ack lib (`src/lib/deliveryAck/generateDeliveryAck.ts`).

Mou-system's company config (MAF Technologies, dual-GSTIN MTPL/MH + MTPL/UP, HSN 999294, programme routing) is co-located at `src/lib/mouSystem/company.json` and read only by the mouSystem-namespace libs.

The two shapes are fundamentally incompatible: Ops carries `legalEntity` + flat `address[]` + `accountDetails[]` + `paymentTerms` + `gstRate`; mou-system carries `entities: {MH: {gstin, address, hsn, piPrefix, ...}, UP: {...}}` + `programmeRouting`. Consolidating now would break Ops's parallel-build flows.

**At cutover:** consolidate by either expanding Ops's `config/company.json` to a true superset (carrying both single-entity Ops fields + dual-GSTIN entity routing + programme routing) or by migrating Ops's dispatch / PI / delivery-ack libs to consume the mouSystem shape. Decision parked for the Gate 5 cutover review.

## 11. Source-byte non-equivalence with gsl-mou-system

The `src/lib/mouSystem/` files are **verbatim functional ports** but **not byte-identical** to the gsl-mou-system source. Two intentional deltas:

1. **20 em-dashes stripped to colons** in source comments + one company.json `_comment` field, per the Ops `scripts/docs-lint.sh` em-dash-zero policy. Stripping was uniform (the U+2014 character to `:`); reading flow is preserved. If we ever do a byte-for-byte diff for parity verification post-cutover, expect 20 lines to differ between Ops's `src/lib/mouSystem/*` and `gsl-mou-system/src/lib/*` for this reason.

2. **5 small TS-target compatibility edits** (regex `/u` flag drops in `installments.ts`, `snapshot.ts`, `vex.ts`, `reconcile.ts`; `Set/Map` iterator wrapping `[...set]` → `Array.from(set)` in `vex.ts` and `reconcile.ts`). Same idioms Ops's own libs already use; no behaviour change.

3 `_`-prefix edits for ESLint unused-arg rules (`reconcile.ts:318` `_mou`, `installments.ts:92` `_fallbackYear`, `reconcile.test.ts:296` `_m`) and one optional-chain non-null-assert fix in `lifecycleReplay.test.ts`. None affect runtime semantics.

`scripts/docs-lint.sh` excludes `src/lib/mouSystem/` from the British-English check (alongside `src/components/ui/`) because the `docx` library's API parameter `color` is not user-facing and renaming would diverge from the verbatim port. This exclusion lives in the script under a documented condition; cutover-time consolidation should review whether to keep the exclusion or migrate the docx library's API to British spelling (the latter is impractical, so the exclusion is expected to persist).

These deltas are part of the parallel-namespace strategy and are reverted only if and when the mouSystem libs collapse into Ops's primary lib tree post-cutover.
